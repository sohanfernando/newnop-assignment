...
public class TaskServiceImpl implements TaskService {
  ...
  public Page<TaskResponse> getAllTasks(
      TaskStatus status, Long ownerId, Pageable pageable, String email) {
    User user = getUser(email);

    if (user.getRole() == Role.ADMIN) {
      return taskRepository.findWithFilters(status, ownerId, pageable).map(this::mapToResponse);
    } else {
      return taskRepository
          .findWithFilters(status, user.getId(), pageable)
