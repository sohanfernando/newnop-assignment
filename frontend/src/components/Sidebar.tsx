...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 flex w-[232px] shrink-0 -translate-x-full flex-col border-r border-(--color-border) bg-[oklch(0.995_0.003_80)] p-4 transition-transform duration-200 md:static md:translate-x-0 ${
          isOpen ? 'translate-x-0' : ''
        }`}
      >
        <div className="flex items-center justify-between gap-2.5 px-1 pb-6">
          ...
          <button
            ...
            className="flex shrink-0 items-center rounded-md p-1.5 text-(--color-text-muted) hover:bg-(--color-border-soft) md:hidden"
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M1.5 1.5l13 13M14.5 1.5l-13 13" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
            </svg>
          </button>
        </div>
