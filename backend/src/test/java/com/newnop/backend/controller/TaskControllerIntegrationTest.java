...
class TaskControllerIntegrationTest {
    ...
    void createTask_thenGetById_asOwner_succeeds() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks/" + id).header("Authorization", token))
                .andExpect(status().isOk())
    ...
    void getTaskById_asDifferentNonAdminUser_isForbidden() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks/" + id).header("Authorization", otherToken))
                .andExpect(status().isForbidden());
    }
    ...
    void getTaskById_asAdmin_canViewAnyUsersTask() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks/" + id).header("Authorization", adminToken()))
                .andExpect(status().isOk())
    ...
    void listTasks_asRegularUser_onlySeesOwnTasks() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks").header("Authorization", user1Token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content.length()").value(2))
                .andExpect(jsonPath("$.data.content[*].ownerUsername",
    ...
    void listTasks_filteredByStatus_onlyReturnsMatchingTasks() throws Exception {
        ...
        mockMvc.perform(put("/api/tasks/" + doneId)
                .header("Authorization", token)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void updateTask_asAdmin_canUpdateAnyUsersTask() throws Exception {
        ...
        mockMvc.perform(put("/api/tasks/" + id)
                        .header("Authorization", adminToken())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(taskPayload("Edited by admin", "IN_PROGRESS", LocalDate.now().plusDays(5))))
                .andExpect(status().isOk())
    ...
    void deleteTask_asNonOwnerNonAdmin_isForbidden() throws Exception {
        ...
        mockMvc.perform(delete("/api/tasks/" + id).header("Authorization", otherToken))
                .andExpect(status().isForbidden());
    }
    ...
    void deleteTask_asOwner_removesTask() throws Exception {
        ...
        mockMvc.perform(delete("/api/tasks/" + id).header("Authorization", token))
                .andExpect(status().isOk())
